...
namespace PSOLib
{
    public enum emCyclic_DType
    {
        Individual = 0,
        Explosion = 1,
    }

    public class Cyclic_D
    {
        #region "Events"
        /// <summary>
        ...
        protected ConstractResult GetConstaint(PSOTuple Curr)
        {
            return GetConstraintResult(Curr);
        }
        //protected double GetFitness(PSOTuple Curr)
        //{
        //    return Fitness(Curr);
        //}
        #endregion
        ...
        public Cyclic_D(int SwarmSize, double[] MaxX, double[] MinX, double[] MaxV, double[] MinV, ParticleDelegateDouble fitness,
                   ...
                   double C1 = 2.0, double C2 = 2.0, double W = 0.1, double Ws = 0.9, double We = 0.5, bool AutoW = true)
        {
            ...
            StagThreshold = (int)(MaxGen * 0.001);
                ...
                */
                ...
                for (int i2 = 0; i2 < _swarm.Particle.Length; i2++)
                {
                    ...
                    for (int iX = 0; iX < Curr.X.Length; iX++)
                    {
                        ...
                        if (Curr.X[iX] < _MinX[iX] || Curr.X[iX] > _MaxX[iX])
                        {
                            Curr.X[iX] = _swarm.GroupBest.X[iX] + (_swarm.Particle[i2].ParticleBest.X[iX]
                                         - _swarm.GroupBest.X[iX]) * RAND_SEED.NextDouble() * 0.1;
                    ...
                    if (DoCheckParticle(Curr))
                    {
                        ...
                        // 6.更新PB/GB
                        if (UpdateFitness(Curr, ParticleBest)) 
                            bEvolved = true;
                    }
                    ...
                    if (_ParticleCyclicStatus[i2] == 1)
                    {
                        ...
                        if (_ParticalStag1[i2] > this.StagThreshold && RAND_SEED.NextDouble() < MutateRate)
                        {
                    ...
                    else
                    {
                        ...
                        double NormalizeStag = (double)_ParticalStag2[i2] / (double)(this.StagThreshold);
        ...
        protected virtual void CalcVelocity(ParticleUnit particle, PSOTuple gb, CParam Param)
        {
            ...
            for (int i1 = 0; i1 < Curr.Velocity.Length; i1++)
            {
                ...
                else
                {
                    ...
                    double dValue = Curr.W * Curr.Velocity[i1] + Curr.C1 * r1 * (dLocalBest - dCurr) + Curr.C2 * r2 * (dGlobalBest - dCurr);
        ...
        // 6.更新PB/GB
        protected virtual bool UpdateFitness(PSOTuple Curr, PSOTuple ParticleBest)
        {
            ...
            {
                _swarm.GroupBest.LastFitness = _swarm.GroupBest.Fitness;
